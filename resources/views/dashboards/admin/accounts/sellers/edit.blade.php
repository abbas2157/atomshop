    ...
    <div class="az-content pd-y-20 pd-lg-y-30 pd-xl-y-40">
        <div class="container">
            ...
            <div class="az-content-body pd-lg-l-40 d-flex flex-column">
                ...
                <form id="product-form-name" method="POST" action="{{ route('admin.sellers.update', $seller->id) }}"
                            ...
                            <div class="row row-sm">
                                <div class="col-lg">
                                    ...
                                    <select class="form-control" name="verified">
                                        <option value="1"
                                            {{ old('verified', $seller->verified ?? '') == '1' ? 'selected' : '' }}>Yes
                                        </option>
                                        <option value="0"
                                            {{ old('verified', $seller->verified ?? '') == '0' ? 'selected' : '' }}>No
                                        </option>
                                    </select>
                                </div>
                ...
                onStepChanging: function(event, currentIndex, newIndex) {
                    if (currentIndex < newIndex) {
                        if (currentIndex === 0) {
                            ...
                            if (business_name.isValid() && name.isValid() && cnic_number.isValid() &&
                    ...
                    success: function(data) {
                        ...
                        $.each(data, function(key, value) {
                            $('#area_id').append('<option value="' + value.id + '">' +
                                value.title + '</option>');
                            selectedAreas[city_id] = selectedAreas[city_id] || [];
                            selectedAreas[city_id].push(value.id);
                        });
                        repopulateSelectedAreas(city_id, {!! json_encode($active_areas_ids) !!});
                    }
                });
            });

            repopulateSelectedAreas($('#city_id').val(), {!! json_encode($active_areas_ids) !!});
        });

        function repopulateSelectedAreas(city_id, active_areas_ids) {
            $.each(selectedAreas[city_id], function(index, area_id) {
                if (active_areas_ids.includes(area_id)) {
