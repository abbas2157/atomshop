                    ...
                    <div class="col-lg mt-2">
                        <div class="input-group">
                            ...
                            <span class="input-group-btn" title="Clear Search">
                                <a href="{{ route('admin.customers.index') }}" class="btn btn-warning" type="submit" style="padding: 8px 20px; color:white;"><i class="far fa-arrow-alt-circle-left"></i></a>
                            </span>
                            <span class="input-group-btn" title="Create New">
                                <a href="{{ route('admin.customers.create') }}" class="btn btn-info" type="submit" style="padding: 8px 20px; color:white;"><i class="typcn typcn-document-add"></i></a>
                            </span>
                        </div>
                    </div>
                </div>
                        ...
                        @if($customers->isNotEmpty())
                            @foreach($customers as $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('admin.customers.edit',$item->uuid) }}">View</a> |
                                        <a href="{{ route('admin.customers.edit',$item->uuid) }}">Edit</a> | 
                                        <a href="javascript:;" id="delete-btn">Delete</a>
                                        <form id="delete-form" action="{{ route('admin.customers.destroy', $item->uuid) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
