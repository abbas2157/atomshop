                        ...
                        <div class="col-lg">
                            <div class="input-group">
                                ...
                                <span class="input-group-btn" title="Clear Search">
                                    <a href="{{ route('admin.areas.index') }}" class="btn btn-warning" type="submit" style="padding: 8px 20px;"><i class="far fa-arrow-alt-circle-left"></i></a>
                                </span>
                                <span class="input-group-btn" title="Create New">
                                    <a href="{{ route('admin.areas.create') }}" class="btn btn-info" type="submit" style="padding: 8px 20px;"><i class="typcn typcn-document-add"></i></a>
                                </span>
                            </div>
                        </div>
                    </div>
                            ...
                            @if($areas->isNotEmpty())
                                @foreach($areas as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.areas.edit',$item->id) }}">View</a> |
                                            <a href="{{ route('admin.areas.edit',$item->id) }}">Edit</a> | 
                                            <a href="javascript:;" id="delete-btn">Delete</a>
                                            <form id="delete-form" action="{{ route('admin.areas.destroy', $item->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </td>
